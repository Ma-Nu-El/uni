...
:END:
:python:
#+PROPERTY: header-args:python :session code :exports both :results output
:end:
...
La librería =datetime= de =python= nos
asiste en la conversión de decimales a
valores de tiempo.

#+begin_src python :session code :exports code :results silent
import datetime
#+end_src
...
round((1 - y_2$RO)*100,3)
#+end_src

#+RESULTS: 2_a
: 16.667

Así, la proporción de tiempo que el servidor está
ocupado es aproximadamente un
call_2_a()[:results value] {{{results(=16.667=)}}} %.
...
Y como estamos trabajando en minutos, entonces
dicho valor se interpreta como:

#+begin_src python :session code :var input=2_b
time = float(input)
...
efectivamente perciba una fila es aproximadamente
call_3_a()[:results value] {{{results(=44.444=)}}}%.
...
y como estamos hablando de horas eso se traduce a
que el cliente está esperando en cola aproximadamente

#+begin_src python :session code :var input=3_c
time = float(input)
...
- Probabilidad de cola:
  ...
  2) y obtenemos el complemento:
     ...
     call_4_b()[:results value] {{{results(=44.444=)}}} %.
...
Y como estamos hablando de minutos, ese valor
se traduce a que el avión en promedio va a
esperar la orden de aterrizaje aproximadamente:

#+begin_src python :session code :var input=5_a
...
procesos pendientes es:

#+begin_src R :session code :results value
paste(c(round((1-y_14$RO)*100,5)),"%")
#+end_src
...
Y como hablamos de horas, entonces el
tiempo promedio que un vehículo pasa en
el taller es aproximadamente

#+begin_src python :session code :var input=14_c
time = round(float(input),2)
print(datetime.timedelta(hours=time))
#+end_src
...
Y como estamos hablando de horas, entonces
el tiempo promedio que un vehículo espera
en cola es aproximadamente

#+begin_src python :session code :var input=14_d
time = round(float(input),3)
print(datetime.timedelta(hours=time))
#+end_src
...
Por lo tanto si consideramos dicho porcentaje
de un turno de 8 horas equivalentes a
\(8*60=480 \) minutos obtenemos:
...
Así, de un turno de 8 horas, el computador
estará ocioso aproximadamente

#+begin_src python :session code :var input=15_b
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
#+end_src
...
- W: tiempo medio de permanencia de un proceso
  ...
  #+begin_src python :session code :var input=15_c_W
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
  #+end_src
...
- Wq: tiempo medio de permanencia de un proceso
  ...
  #+begin_src python :session code :var input=15_c_Wq
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
  #+end_src
...
#+begin_src python :session code :var input=16_b
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
#+end_src
...
#+begin_src python :session code :var input=16_c
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
#+end_src
...
#+begin_src R :session code :results value
caseta<-round(y_16_caseta$RO,5)
nave<-round(y_16_nave$RO,5)
oficina<-round(y_16_oficina$RO,5)

print(paste(caseta,nave,oficina))
#+end_src

#+RESULTS:
: 0.99379 0.99379 0.75
...
de 30 minutos en promedio y/o que los
registros en realidad no alcanzan las 100
pruebas por día.
...
El promedio de aspirantes es 6.25,
por lo que para que el sistema
esté lleno los aspirantes
en el sistema en cualquier momento
deben ser 7 o más, que es lo mismo que
el complemento de 0 hasta 6.

Procedimiento similar al [[id:02bb4cf7-f019-46f4-a4e2-8d71527122ad][ejercicio 3]].
...
Dicha probabilidad es aproximadamente un
call_y_17_b()[:results value] {{{results(=29.83=)}}} %.
...
- Los procesos entrantes son las máquinas cuando
  terminan su proceso.
- El servicio es aquel que presta el mantenedor
  a las máquinas.
- La cantidad de máquinas es la capacidad
  total del sistema.
...
Eso hace referencia al total de máquinas
en el sistema.
...
#+begin_src python :session code :var input=18_c
time = round(float(input),2)
print(datetime.timedelta(minutes=time))
#+end_src
...
- Lenguaje para cálculo de parámetros:
